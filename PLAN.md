...
- [ ] Port codescan self-suiciding watcher to chatscan (`chatscan watch`): (1) pure retirement.zig policy [self-kill decision, idle-limit parse, activity, format]; (2) watch.zig polling loop over indexAll with injected clock; (3) CLI `watch` cmd + --interval/--idle-timeout + config keys watcher_interval/watcher_idle_timeout + SIGINT stop. Mirrors ~/Code/codescan/src/{retirement,watcher}.zig.
